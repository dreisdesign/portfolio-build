...
if (import.meta.url.endsWith(process.argv[1])) {
  startServer(BUILD_DIR).then((proceed) => {
    if (proceed) {
      ...
      // Improved cancellation messaging and exit code
      console.log('ℹ️ Build cancelled by user');
      logger.info('Deployment stopped safely.');
      process.exit(0); // Clean exit instead of error exit
    }
  });
}
